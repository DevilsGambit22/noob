(() => {
  ...
  async function loadNewestMembers() {
    ...
    try {
      ...
      rawMembers.forEach((entry, sourceIndex) => {
        const member = normalizeClubMember(entry, sourceIndex);
      ...
      const hasClubJoinDates = members.some((member) => member.joined > 0);
      const newest = members
        .sort((a, b) => hasClubJoinDates
          ? (b.joined - a.joined || a.sourceIndex - b.sourceIndex)
