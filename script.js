(() => {
  ...
  function normalizeClubMember(entry, sourceIndex) {
    if (typeof entry === "string") {
      return { username: entry, joined: 0, sourceIndex };
    }
  ...
  async function loadNewestMembers() {
    ...
    try {
      ...
      const rawMembers = Array.isArray(clubData.all_time) ? clubData.all_time : [];

      const members = rawMembers
        .map((entry, sourceIndex) => normalizeClubMember(entry, sourceIndex))
        .filter((member) => member?.username);

      // Chess.com includes each club member's club-join timestamp in all_time.
      // Sort that list newest-first, exactly like our other dashboard boards.
      const hasJoinDates = members.some((member) => member.joined > 0);
      const newest = [...members]
        .sort((a, b) => hasJoinDates
          ? (b.joined - a.joined || b.sourceIndex - a.sourceIndex)
      ...
      membersBoard.innerHTML = newest.map((member, index) => {
        ...
        const memberLabel = member.joined ? joinedLabel(member.joined) : "Republic member";
        ...
        return `<a class="member-card" style="--member-delay:${index * 65}ms" href="${escapeHtml(profileUrl)}" target="_blank" rel="noopener noreferrer">
          ...
          <span class="member-meta">${title}${escapeHtml(memberLabel)}</span>
        </a>`;
      }).join("");

      membersStatus.textContent = `Showing ${newest.length} members`;
    } catch (error) {
